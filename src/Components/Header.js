...
const Header = ({ movieId }) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        movieId ? navigate("/browse/" + movieId) : navigate("/browse");
      } else {
        // User is signed out
        dispatch(removeUser());
        navigate("/");
  ...
  return (
    <div className=" absolute w-full px-16  flex justify-between bg-gradient-to-b from-black z-10">
        ...
        <div className=" flex">
          {gptSearchBtn && (
            <select
              ...
              className=" bg-purple-500 rounded-lg my-8 mx-2 px-2 py-1"
            >
          ...
          <button
            ...
            className=" bg-purple-500 hover:bg-purple-600 rounded-lg my-8 px-2 py-1"
          >
            {gptSearchBtn ? "Home" : "GPTSearch"}
          </button>
          <img className=" w-20 my-2 p-4   " src={USER_ICON} alt="UserIcon" />
          <button
            onClick={handleSignOut}
            className=" bg-red-600 hover:bg-red-700 text-white my-8 px-2 rounded-md"
          >
            Sign Out
          </button>
        </div>
